...
logging.basicConfig(filename='.log/reg.log', format='%(asctime)s - %(message)s', level=logging.INFO, datefmt='%d-%b-%y %H:%M:%S')
...
bot = commands.Bot(command_prefix = '!', description=f"Registration Desk for {event_name}", help_command=None)
...
async def register(ctx, *, info):
    ...
    if speaker is None:
        logging.info(f'FAIL: Cannot find request form user {ctx.author} with name={info[0]}, ticket_no={info[1]}')
        await ctx.send(f"{ctx.author.mention} Sorry cannot find the ticket #{info[1]} with name: {info[0]}.\nPlease check and make sure you put down your full name same as the one you used in registering your ticket then try again.")
    else:
        logging.info(f'SUCCESS: Register user {ctx.author} with name={info[0]}, ticket_no={info[1]}')
        ...
        if speaker:
            ...
            await ctx.send(f"Welcome {ctx.author.mention}, you now have the speaker and attendee role.")
        else:
            await ctx.send(f"Welcome {ctx.author.mention}, you now have the attendee role.")
